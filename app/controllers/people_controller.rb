class PeopleController < GenericPeopleController
  ...
  def create
    ...
    if create_from_dde_server
      ...
      DDEService.create_footprint(PatientService.get_patient(person).national_id, Location.location(session[:location_id]).name)
    ...
    if params[:identifier]
      ...
      if found_person
        ...
        if params[:relation]
          ...
          DDEService.create_footprint(PatientService.get_patient(found_person).national_id, Location.name(session[:location_id]).name)
          print_and_redirect("/patients/national_id_label?patient_id=#{found_person.id}", next_task(found_person.patient)) and return
