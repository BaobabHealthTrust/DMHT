...
def self.treatments(patient)
  ...
  def self.diabetes_number(patient)
   ...
   def self.complications_label(patient, user_id,site_prefix)
    ...
    label.draw_multi_text("#{site_prefix} DM CLINIC: #{patient_bean.name.to_s.titleize.delete("'")} (#{patient_bean.national_id_with_dashes}#{dc_number}) ")
